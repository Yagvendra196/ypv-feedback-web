angular.module('starter.controllers', ['ionic','ngCordova.plugins.inAppBrowser'])
        ...
        .controller('loginCtrl', function ($rootScope,$scope, $timeout, $ionicLoading, $ionicPopup, $state, $http,$cordovaInAppBrowser) {
            ...
            $scope.forgotPasswordSubmit = function (forgotPasswordForm) {
                // console.log(forgotPasswordForm.accountemail.$modelValue);
                let accountemail = forgotPasswordForm.accountemail.$modelValue;
        ...
        .controller('MyCtrl', function($scope, $cordovaInAppBrowser) {
         ...
         .controller('dashboardCtrl', function ($scope, $ionicPopup, $timeout, $http, $ionicLoading) {
            $scope.options = {
                location: 'yes',
                clearcache: 'yes',
                toolbar: 'no'
            ...
            $scope.showAlert = function() {
                var alertPopup =  $ionicPopup.show({
                    ...
                    template: 'Please go to the Play Store/App Store and update the app to the latest version.',
                    buttons: [
                            {
                            text: '<b>Update</b>',
                            type: 'button-positive',
                            onTap: function(e) {
                                window.open('market://details?id=ypv.spirt.buddy', '_system', $scope.options);
                                e.preventDefault();
                            }
                            }
                        ],
            ...
            $scope.getAppCurrentVersion = function(){
                ...
                $http.post(url, {'access_token': window.localStorage.getItem('auth_token')}).success(function (response)
                {
                    // $timeout(function () {
                        ...
                        if (response.response == 'S') {
                            console.log(response.version_name);
                            if(currentAppVersion === response.version_name){
                                this.feedbackBuddies_data = response.data;
                            }else{
                                $scope.showAlert();
                            }
                        }
                        ...
                        $http.post(url, {'feedback_type': 'Weekly', 'access_token': window.localStorage.getItem('auth_token'), 'idWeek': $scope.idWeek, 'spiritual_buddie_user_id': $stateParams.id, 'purpose': 'edit', 'use_date_filter':1 }).success(function (res)
                        {
                           if (res.response == 'S') {   
                                    angular.forEach(res.data, function (value, key) {
                                        ...
                                        //Added due to change in value
                                        if (value.feedback_field_id === '1' && value.user_feedback_field_value !== '' ) {
                                            if(value.user_feedback_field_value === 'Prep' || value.user_feedback_field_value == 'Preliminary'){
                                                value.user_feedback_field_value = 'Preliminary'
                                            }
                                        }
                ...
                $http.post(url, {'feedback_type': 'Weekly', 'access_token': window.localStorage.getItem('auth_token'), 'idWeek': $scope.idWeek, 'spiritual_buddie_user_id': $scope.spiritualBuddieUserID, 'purpose': 'view', 'use_date_filter':1}).success(function (res)
                {
                    $timeout(function () {
                        ...
                        if (res.response == 'S') {
                            ...
                            angular.forEach(res.data, function (value, key) {
                                var k = 'feedback_field_'+value.feedback_field_id;
                                //Added this condition due to change in value
                                if (value.feedback_field_id === '1' && value.user_feedback_field_value !== '' ) {
                                    if(value.user_feedback_field_value === 'Prep' || value.user_feedback_field_value == 'Preliminary'){
                                        value.user_feedback_field_value = 'Preliminary'
                                    }
