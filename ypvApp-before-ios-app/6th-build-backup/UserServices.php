   ...
   // user login
    ...
    function getAddBuddies_post() {
        ...
        if (isset($userID) && !empty($userID)) {
            ...
            $user_buddies_not = $this->db->query("SELECT u.user_id, u.email, u.first_name, u.last_name, ur.role_id, ur.status, mr.role_name, usb.spiritual_buddie_user_id, usb.status ,up.* 
                FROM `users` as u
                ...
                WHERE  ur.role_id = 3 and u.user_id != '".$userID."' AND u.user_id != '1' AND usb.user_id IS NULL AND u.user_id in  (SELECT DISTINCT user_id FROM `user_owners` WHERE `owner_user_id` in (SELECT `owner_user_id` FROM `user_owners` WHERE user_id = '".$userID."')) ORDER BY u.user_id ASC")->result();
            ...
            if (isset($userID) && !empty($userID)) {
                ...
                if ($feedback_type=='Monthly') {
                        ...
                        if ($this->post('purpose') == 'edit') {
                            ...
                            $where = array('ufb.feedback_type'=>$feedback_type,'ufb.user_id'=>$userID,'spiritual_buddie_user_id'=>$spiritual_buddie_user_id);                    
                        }
